...
const float WXCoreLayoutNode::FLEX_DEFAULT = 1;
const float WXCoreLayoutNode::FLEX_GROW_DEFAULT = 0;
const float WXCoreLayoutNode::MAX_SIZE = NAN;
...
void WXCoreLayoutNode::measureHorizontal(float width, float height) {
  ...
  for (int i = 0; i < childCount; i++) {
    ...
    if (child->mAlignSelf == WXCore_AlignSelf_Stretch) {
    ...
    largestHeightInRow = max_num<float>(largestHeightInRow,
                                        child->mComputedHeight + child->getMarginTop() + child->getMarginBottom());

    // Determine if a wrap is required (add a new flexline)
    if (isWrapRequired(width, flexLine->mMainSize,
                       child->mComputedWidth + child->getMarginLeft() + child->getMarginRight()
    )) {
      ...
      flexLine->mMainSize = getPaddingLeft() + getPaddingRight();
      largestHeightInRow = child->mComputedHeight + child->getMarginTop() + child->getMarginBottom();
    } else {
      flexLine->mItemCount++;
    }

    flexLine->mMainSize += child->mComputedWidth + child->getMarginLeft() + child->getMarginRight();
    flexLine->mTotalFlexGrow += child->mFlexGrowValue;
  ...
  determineMainSize(mFlexDirection, width, height);

  // Determines the cross size (Calculate the length along the cross axis).
  determineCrossSize(mFlexDirection, width, height, getPaddingTop() + getPaddingBottom());
...
void WXCoreLayoutNode::measureVertical(float width, float height) {
  ...
  int childCount = getChildCount();
  float largestWidthInColumn = NAN;
  float totalCrossSize = 0;
  ...
  for (int i = 0; i < childCount; i++) {
    ...
    if (isWrapRequired(height, flexLine->mMainSize,
      ...
      flexLine->mItemCount = 1;
      flexLine->mMainSize = getPaddingTop() + getPaddingBottom();
      largestWidthInColumn = child->mComputedWidth + child->getMarginLeft() + child->getMarginRight();
    } else {
      flexLine->mItemCount++;
    }

    flexLine->mMainSize += child->mComputedHeight + child->getMarginTop() + child->getMarginBottom();
    flexLine->mTotalFlexGrow += child->mFlexGrowValue;
  ...
  // Determines the cross size (Calculate the length along the cross axis).
  determineCrossSize(mFlexDirection, width, height, getPaddingLeft() + getPaddingRight());
...
void WXCoreLayoutNode::checkSizeConstraints(WXCoreLayoutNode *node) {
  ...
  float childWidth = node->mComputedWidth;
  float childHeight = node->mComputedHeight;

  if (node->mComputedWidth < node->mMinWidth) {
    needsMeasure = true;
    childWidth = node->mMinWidth;
  } else if (node->mComputedWidth > node->mMaxWidth) {
    needsMeasure = true;
    childWidth = node->mMaxWidth;
  }

  if (childHeight < node->mMinHeight) {
    needsMeasure = true;
    childHeight = node->mMinHeight;
  } else if (childHeight > node->mMaxHeight) {
    needsMeasure = true;
    childHeight = node->mMaxHeight;
  }
...
int WXCoreLayoutNode::expandFlexItems(float widthMeasureSpec, float heightMeasureSpec, WXCoreFlexLine *flexLine,
                                      WXCoreFlexDirection flexDirection, float maxMainSize, float paddingAlongMainAxis,
  ...
  for (int i = 0; i < flexLine->mItemCount; i++) {
    ...
    if (isMainAxisDirectionHorizontal(flexDirection)) {
      ...
      if (!mChildrenFrozen[childIndex]) {
        float rawCalculatedWidth = child->mComputedWidth + unitSpace * child->mFlexGrowValue;
        ...
        largestCrossSize = max_num<float>(largestCrossSize, child->mComputedHeight
      ...
      if (!mChildrenFrozen[childIndex]) {
        float rawCalculatedHeight = child->mComputedHeight + unitSpace * child->mFlexGrowValue;
        ...
        largestCrossSize = max_num<float>(largestCrossSize, child->mComputedWidth
...
float WXCoreLayoutNode::getChildWidthMeasureSpec(float widthMeasureSpec, WXCoreLayoutNode *node, float padding) {
  ...
  if (childWidth > node->mMaxWidth) {
    childWidthMeasureSpec = node->mMaxWidth;
  } else if (childWidth < node->mMinWidth) {
    childWidthMeasureSpec = node->mMinWidth;
  }
...
float WXCoreLayoutNode::getChildHeightMeasureSpec(float heightMeasureSpec, WXCoreLayoutNode *node, float padding) {
  ...
  if (childHeight > node->mMaxHeight) {
    childHeightMeasureSpec = node->mMaxHeight;
  } else if (childHeight < node->mMinHeight) {
    childHeightMeasureSpec = node->mMinHeight;
  }
  ...
  if (alignItems == WXCore_AlignItems_Stretch) {
    ...
    for (WXCoreFlexLine *flexLine : mFlexLines) {
      for (int i = 0; i < flexLine->mItemCount; i++, viewIndex++) {
        ...
        if (child->mAlignSelf != WXCore_AlignSelf_Auto &&
            child->mAlignSelf != WXCore_AlignSelf_Stretch) {
          continue;
        }
...
void WXCoreLayoutNode::stretchViewVertically(WXCoreLayoutNode *node, float crossSize) {
  ...
  node->measure(node->mComputedWidth, newHeight);
}
...
void WXCoreLayoutNode::stretchViewHorizontally(WXCoreLayoutNode *node, float crossSize) {
  ...
  node->measure(newWidth, node->mComputedHeight);
}
...
bool WXCoreLayoutNode::isWrapRequired(float maxSize, float currentLength, float childLength) {
  if (mFlexWrap == WXCore_Wrap_NoWrap) {
    return false;
  }
...
void WXCoreLayoutNode::onLayout(float left, float top, float right, float bottom) {
  switch (mFlexDirection) {
    ...
    case WXCore_Flex_Direction_Row_Reverse:
      ...
      break;
    case WXCore_Flex_Direction_Column:
      layoutVertical(mFlexWrap == WXCore_Wrap_WrapReverse, false, left, top, right, bottom);
      break;
    case WXCore_Flex_Direction_Column_Reverse:
      layoutVertical(mFlexWrap == WXCore_Wrap_WrapReverse, true, left, top, right, bottom);
      break;
//    default:
...
//            "Invalid justifyContent is set: " + mJustifyContent);
    ...
    for (int j = 0; j < flexLine->mItemCount; j++) {
      ...
      if (mFlexWrap == WXCore_Wrap_WrapReverse) {
        if (isRtl) {
          layoutSingleChildHorizontal(child, flexLine, mFlexWrap, mAlignItems,
                                      childRight - child->mComputedWidth,
                                      childBottom - child->mComputedHeight, childRight,
                                      childBottom);
        } else {
          layoutSingleChildHorizontal(child, flexLine, mFlexWrap, mAlignItems,
                                      childLeft, childBottom - child->mComputedHeight,
                                      childLeft + child->mComputedWidth,
                                      childBottom);
        }
      } else {
        if (isRtl) {
          layoutSingleChildHorizontal(child, flexLine, mFlexWrap, mAlignItems,
                                      childRight - child->mComputedWidth, childTop,
                                      childRight, childTop + child->mComputedHeight);
        } else {
          layoutSingleChildHorizontal(child, flexLine, mFlexWrap, mAlignItems,
                                      childLeft, childTop,
                                      childLeft + child->mComputedWidth,
                                      childTop + child->mComputedHeight);
        }
      }

      childLeft += child->mComputedWidth + spaceBetweenItem + child->getMarginRight();
      childRight -= child->mComputedWidth + spaceBetweenItem + child->getMarginLeft();
      currentViewIndex++;
    }
...
void WXCoreLayoutNode::layoutSingleChildHorizontal(WXCoreLayoutNode *node, WXCoreFlexLine *flexLine,
  ...
  if (node->mAlignSelf != WXCore_AlignSelf_Auto) {
    ...
    alignItems = static_cast<WXCoreAlignItems>(node->mAlignSelf);
  }
  ...
  switch (alignItems) {
    ...
    case WXCore_AlignItems_Baseline:
      if (flexWrap != WXCore_Wrap_WrapReverse) {
        ...
        float marginBottom = flexLine->mMaxBaseline - node->mComputedHeight + node->getBaseline();
    ...
    case WXCore_AlignItems_Flex_End:
      if (flexWrap != WXCore_Wrap_WrapReverse) {
        node->layout(left,
                     top + crossSize - node->mComputedHeight - node->getMarginBottom(),
        ...
        node->layout(left, top - crossSize + node->mComputedHeight + node->getMarginTop(),
                     right, bottom - crossSize + node->mComputedHeight + node->getMarginTop());
      }
      break;
    case WXCore_AlignItems_Center:
      float topFromCrossAxis = (crossSize - node->mComputedHeight
                                + node->getMarginTop() - node->getMarginBottom()) / 2;
      if (flexWrap != WXCore_Wrap_WrapReverse) {
        node->layout(left, top + topFromCrossAxis,
                     right, top + topFromCrossAxis + node->mComputedHeight);
      } else {
        node->layout(left, top - topFromCrossAxis,
                     right, top - topFromCrossAxis + node->mComputedHeight);
      }
...
//            "Invalid justifyContent is set: " + mJustifyContent);
    ...
    for (int j = 0; j < flexLine->mItemCount; j++) {
      ...
      if (isRtl) {
        ...
        if (fromBottomToTop) {
          layoutSingleChildVertical(child, flexLine, false, mAlignItems,
                                    childLeft, childBottom - child->mComputedHeight,
                                    childLeft + child->mComputedWidth, childBottom);
        } else {
          layoutSingleChildVertical(child, flexLine, false, mAlignItems,
                                    childLeft, childTop,
                                    childLeft + child->mComputedWidth,
      ...
      childTop += child->mComputedHeight + spaceBetweenItem + child->getMarginBottom();
      childBottom -= child->mComputedHeight + spaceBetweenItem + child->getMarginTop();
      currentViewIndex++;
    }
...
void WXCoreLayoutNode::layoutSingleChildVertical(WXCoreLayoutNode *node, WXCoreFlexLine *flexLine, bool isRtl,
  ...
  switch (alignItems) {
    ...
    case WXCore_AlignItems_Flex_End:
      if (!isRtl) {
        node->layout(left + crossSize - node->mComputedWidth - node->getMarginRight(),
                     top, right + crossSize - node->mComputedWidth - node->getMarginRight(),
                     bottom);
      } else {
        // If the flexWrap == FLEX_WRAP_WRAP_REVERSE, the direction of the
        // flexEnd is flipped (from mStyleLeft to mStyleRight).
        node->layout(left - crossSize + node->mComputedWidth + node->getMarginLeft(), top,
                     right - crossSize + node->mComputedWidth + node->getMarginLeft(),
                     bottom);
      }
      break;
    case WXCore_AlignItems_Center:
      float leftFromCrossAxis = (crossSize - node->mComputedWidth
                                 + node->getMarginLeft()
