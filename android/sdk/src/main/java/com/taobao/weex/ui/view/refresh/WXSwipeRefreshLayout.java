...
public class WXSwipeRefreshLayout extends FrameLayout {
    ...
    private WXRefreshLayout headerView;
    private WXRefreshLayout footerView;
    ...
    public WXSwipeRefreshLayout(Context context) {
        super(context);
        initAttrs(context, null);

    }

    public WXSwipeRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        initAttrs(context, attrs);
    }

    public WXSwipeRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initAttrs(context, attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public WXSwipeRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        int resId;
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.WXSwipeRefreshLayout);
        Resources resources = context.getResources();

        //LoadView背景颜色
        resId = ta.getResourceId(R.styleable.WXSwipeRefreshLayout_load_view_bg_color, -1);
        if (resId == -1) {
            mLoadViewBgColor = ta.getColor(R.styleable.WXSwipeRefreshLayout_load_view_bg_color,
                    Color.TRANSPARENT);
        } else {
        ...
        //加载文字颜色
        resId = ta.getResourceId(R.styleable.WXSwipeRefreshLayout_load_text_color, -1);
        if (resId == -1) {
            mLoadViewTextColor = ta.getColor(R.styleable.WXSwipeRefreshLayout_load_text_color,
                    Color.BLACK);
        } else {
        ...
        //进度条背景颜色
        resId = ta.getResourceId(R.styleable.WXSwipeRefreshLayout_progress_bg_color, -1);
        if (resId == -1) {
            mProgressBgColor = ta.getColor(R.styleable.WXSwipeRefreshLayout_progress_bg_color,
                    Color.TRANSPARENT);
        } else {
        ...
        //进度条颜色
        resId = ta.getResourceId(R.styleable.WXSwipeRefreshLayout_progress_bar_color, -1);
        if (resId == -1) {
            mProgressColor = ta.getColor(R.styleable.WXSwipeRefreshLayout_progress_bar_color,
                    Color.RED);
        } else {
        ...
        //下拉刷新文字描述
        resId = ta.getResourceId(R.styleable.WXSwipeRefreshLayout_pull_refresh_text, -1);
        if (resId == -1) {
            mPullRefreshText = ta.getString(R.styleable.WXSwipeRefreshLayout_pull_refresh_text);
        } else {
            mPullRefreshText = resources.getString(resId);
        }

        //上拉加载文字描述
        resId = ta.getResourceId(R.styleable.WXSwipeRefreshLayout_pull_load_text, -1);
        if (resId == -1) {
            mPullLoadText = ta.getString(R.styleable.WXSwipeRefreshLayout_pull_load_text);
        } else {
            mPullLoadText = resources.getString(resId);
        }

        mAutoLoadMore = ta.getBoolean(R.styleable.WXSwipeRefreshLayout_auto_load_more, false);
    }
    ...
    private void setupViews() {
        ...
        if (mPullRefreshEnable) {
            ...
            headerView = new WXRefreshLayout(getContext());
...
//                    getContext().getString(R.string.default_pull_refresh_text) : mPullRefreshText);
        ...
        if (mPullLoadEnable) {
            ...
            footerView = new WXRefreshLayout(getContext());
...
//                    getContext().getString(R.string.default_pull_load_text) : mPullLoadText);
        ...
        Log.i("miomin", "headerview init finish!!");
    }
...
//                getContext().getString(R.string.default_pull_load_text) : mPullLoadText);
    ...
    public WXRefreshLayout getHeaderView() {
        return headerView;
    }
    ...
    public void setmPullLoadEnable(boolean mPullLoadEnable) {
        this.mPullLoadEnable = mPullLoadEnable;
    }

    public boolean ismPullRefreshEnable() {
        return mPullRefreshEnable;
    }

    public void setmPullRefreshEnable(boolean mPullRefreshEnable) {
