(this.nativeLog || function(s) {console.log(s)})('START JS FRAMEWORK 0.26.0-beta.0, Build 2018-04-24 21:31. (Vue: 2.5.13-weex.5, Rax: 0.4.20)');
...
TaskCenter.prototype.send = function send (type, params, args, options) {
  ...
  switch (type) {
    ...
    default:
      console.log(("[JS Framework] send module call to native (" + (this.instanceId) + "): " + module + ", " + method + ", " + (JSON.stringify(args)) + ", " + (JSON.stringify(args))));
...
function moduleGetter (id, module, method) {
  console.log(("[JS Framework] invoke moduleGetter(" + id + ", " + module + ", " + method + ")"));
  var taskCenter = getTaskCenter(id);
  if (!taskCenter || typeof taskCenter.send !== 'function') {
  ...
  return function () {
    ...
    console.log(("[JS Framework] invoke getter module: " + module + ", " + method + ", " + (JSON.stringify(args))));
    return taskCenter.send('module', { module: module, method: method }, args)
  }
}

function moduleSetter (id, module, method, fn) {
  console.log(("[JS Framework] invoke moduleSetter(" + id + ", " + module + ", " + method + ", " + (typeof fn) + ")"));
  var taskCenter = getTaskCenter(id);
  if (!taskCenter || typeof taskCenter.send !== 'function') {
    console.error(("[JS Framework] Failed to find taskCenter (" + id + ")."));
    return null
  ...
  return function (fn) {
    console.log(("[JS Framework] invoke setter module: " + module + ", " + method + ", " + (typeof fn)));
...
WeexInstance.prototype.requireModule = function requireModule (moduleName) {
  ...
  if (!(id && this.document && this.document.taskCenter)) {
    console.log("[JS Framework] Failed to requireModule(\"" + moduleName + "\"), "
  ...
  if (!isRegisteredModule(moduleName)) {
    console.log(("[JS Framework] using unregistered weex module \"" + moduleName + "\""));
    return
  }
  ...
  if (!moduleProxies[proxyName]) {
    ...
    var moduleApis = {};
    console.log(("[JS Framework] define a module: " + moduleName));
    var loop = function ( methodName ) {
      console.log(("[JS Framework] define a module method: " + moduleName + "." + methodName));
...
function getJSFMVersion () {
  ...
  return "0.26.0-beta.0" // eslint-disable-line
}
...
function createInstanceContext(instanceId, __weex_options__, __weex_data__) {
  var instance = instances[instanceId];
  var weex = __weex_options__.weex;
...
var Rax = unwrapExports(framework);
