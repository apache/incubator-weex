    ...
    WeexCoreManager::Instance()->script_thread()->message_loop()->PostTask(
        weex::base::MakeCopyable(
            [pageId = std::unique_ptr<char[]>(copyStr(page_id)),
             ...
             optLen = options_length, result = &ret, e = &event] {
              RenderManager::GetInstance()->CallNativeModule(
...
void CoreSideInSimple::CreateBody(const char *page_id, const char *dom_str,
  ...
  WeexCoreManager::Instance()->script_thread()->message_loop()->PostTask(
      weex::base::MakeCopyable(
          [pageId = std::unique_ptr<char[]>(copyStr(page_id)),
           domStr = std::unique_ptr<char[]>(copyStr(dom_str, dom_str_length))] {
            RenderManager::GetInstance()->CreatePage(pageId.get(), domStr.get())
                ? 0
                : -1;
          }));
}
      ...
      weex::base::MakeCopyable(
          [pageId = std::unique_ptr<char[]>(copyStr(page_id)),
           ...
           dataS = std::unique_ptr<char[]>(copyStr(data, data_length))] {
            RenderManager::GetInstance()->UpdateAttr(pageId.get(), refS.get(),
                                                     dataS.get());
          }));
}
  ...
  WeexCoreManager::Instance()->script_thread()->message_loop()->PostTask(
      weex::base::MakeCopyable(
          [pageId = std::unique_ptr<char[]>(copyStr(page_id)),
           refS = std::unique_ptr<char[]>(copyStr(ref)),
           dataS = std::unique_ptr<char[]>(copyStr(data, data_length))] {
            RenderManager::GetInstance()->UpdateStyle(pageId.get(), refS.get(),
                                                      dataS.get());
          }));
}
